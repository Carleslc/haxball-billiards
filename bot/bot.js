...
const TOKEN = 'thr1.AAAAAGKzUuxieTX3NiF9pQ.JnWfvf0r_fY';
...
const TEAM_LIMIT = 1; // maximum players per team in a game
...
const DRINK_PREPARATION_SECONDS = 2;
...
let AFTER_TURN_SECONDS = 2; // delay to move current player (balls are moving after shot)
...
function updateCurrentPlayer({ changeTeam = true, delayMove = false } = {}) {
  if (N_PLAYERS === 0) {
    resetCurrentPlayer();
...
function nextTurn(delayMove = false) {
  if (playersInGameLength() > 1) {
    ...
    if (delayMove) {
      setTimeout(movePlayer, AFTER_TURN_SECONDS * 1000);
    } else {
      movePlayer();
    }
  }
  ...
  console.log('CURRENT_PLAYER', CURRENT_PLAYER.name, 'FOUL', FOUL);
}
...
function updateCurrentPlayerCollisions() {
  ...
  updatePlayerFlags('cGroup', CURRENT_PLAYER, (cGroup) => {
...
const HOST_POSITION = position(-455, -10);

function moveHostPlayer(host) {
  if (host) {
    // Move host out of the table
    room.setPlayerDiscProperties(host.id, HOST_POSITION);
  }
}
...
function startGame() {
  if (!PLAYING) {
    ...
    const caller = startGame.caller.name;
    const delay = N_PLAYERS > 1 ? WAIT_GAME_START_SECONDS : 0;
...
function stopGame() {
  console.log(stopGame.caller.name, '-> stopGame');
  room.stopGame();
}
...
function setTeams() {
  ...
  function addPlayer(spectator, i, team) {
    setPlayerTeam(spectator, team);
    teams[team].push(spectator);
    changedTeams.add(i);
  }

  function fill(spectator, i) {
    if (teams[nextTeam].length < TEAM_LIMIT) {
      addPlayer(spectator, i, nextTeam);
      return true;
    } else if (teams[otherTeam].length < TEAM_LIMIT) {
      addPlayer(spectator, i, otherTeam);
      return true;
    }
    return false;
  }

  teams[TEAM.SPECTATOR].filter(p => !isAFK(p)).forEach((spectator, i) => {
    if (fill(spectator, i)) {
  ...
  teams[TEAM.SPECTATOR] = teams[TEAM.SPECTATOR].filter(spectator => !changedTeams.has(spectator));
...
function setAuth(player) {
  ...
  // Restore drink
  if (isDrinking(player)) {
    room.setPlayerAvatar(player.id, getDrink(player));
  }
}
...
function incrementTurnTime() {
  if (PLAYING && playersInGameLength() > 1) {
    TURN_TIME++;
...
function updateOnTeamMove(player) {
  if (PLAYING) {
    ...
    if (!playersInGameLength() || (activePlayers().length < 2 && CURRENT_MAP !== 'PRACTICE')) {
      stopGame();
    }
  }
}
...
const CHECK_EVERY_TICKS = TICKS_PER_SECOND / 2;
...
function foul(message, nextTurn = true) {
  ...
  const previousPlayer = CURRENT_PLAYER;
  ...
  if (nextTurn) {
    ...
    if (CURRENT_PLAYER.id !== previousPlayer.id) {
      info(`${CURRENT_PLAYER.name} have ${FOUL} shots now`);
    }
  }
}
...
function gameOver(success = false) {
  ...
  const team = success ? CURRENT_TEAM : getOppositeTeam(CURRENT_TEAM);
  
  notify(`${getTeamIcon(team)} ${getTeamName(team)} wins`, getTeamColor(team), 'bold');
