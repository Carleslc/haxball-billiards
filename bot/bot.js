...
function hasFlag(current, flag) {
  return (current & flag) != 0;
}

function addFlag(current, flag) {
  return current | flag;
}

function removeFlag(current, flag) {
  return current & ~flag;
}

function updatePlayerFlags(field, player, updatedFlags) {
  if (player) {
    const playerDiscFlags = room.getPlayerDiscProperties(player.id)[field];
...
function loadMapProperties() {
  if (CURRENT_MAP_OBJECT) {
    ...
    REMAINING_RED_BALLS = RED_BALLS.length;
    REMAINING_BLUE_BALLS = BLUE_BALLS.length;
...
function updateCurrentPlayer(changeTeam = true) {
  if (N_PLAYERS === 0) {
    ...
    if (playerChanged && playersInGameLength() > 1) {
      nextTurn();
    }
  }
  TURN_TIME = 0;
...
function nextTurn() {
  ...
  const movePlayer = () => {
    ...
    info(`Turn ${teamColor} ${CURRENT_PLAYER.name}`);
  };

  if (isBallMoving(WHITE_BALL)) {
...
function moveCurrentPlayer() {
  if (CURRENT_PLAYER) {
    ...
    if (!isSpawn || !inPlayingArea(CURRENT_PLAYER.position)) {
      let pos;
